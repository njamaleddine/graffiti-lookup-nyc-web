...
describe('ListView.vue', () => {
  ...
  it('shows the correct count in the header', () => {
    ...
    expect(wrapper.find('.count').text()).toContain(`(${items.length} of ${items.length})`);
  });
  ...
  it('filters items by status', async () => {
    ...
    await wrapper.findComponent(StatusFilter).vm.$emit('update:modelValue', 'pending');
    await wrapper.vm.$nextTick();
    expect(wrapper.findAllComponents(ListItem).length).toBe(1);
    expect(wrapper.findComponent(ListItem).props('item').status).toBe('pending');
  });
  ...
  it('filters items by search query (address)', async () => {
    ...
    await wrapper.findComponent(SearchBar).vm.$emit('update:modelValue', 'Broadway');
    await wrapper.vm.$nextTick();
    expect(wrapper.findAllComponents(ListItem).length).toBe(1);
    expect(wrapper.findComponent(ListItem).props('item').address).toBe('456 Broadway');
  });
  ...
  it('shows no results message when filter yields no items', async () => {
    ...
    await wrapper.findComponent(SearchBar).vm.$emit('update:modelValue', 'NotFound');
    await wrapper.vm.$nextTick();
